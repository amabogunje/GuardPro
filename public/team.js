...
export function teamSettings(host,{site,users,reusableUsers=[],api,esc,icon,done,roleOnly=null,allowReuse=false,startAdd=false}) {
  ...
  function edit(member) {
    ...
    const update=()=>{
      ...
      const duplicate=!member&&normalized&&members.find(person=>phoneNumber(person.whatsapp)===normalized);
      form.elements.whatsapp.setCustomValidity(phone&&!normalized?'Enter a valid WhatsApp number':!phone?'Enter a WhatsApp number':duplicate?'This WhatsApp number is already used by '+duplicate.name:'');
      if(!member) status.textContent=!phone?'Enter the required fields.':!normalized?'Enter a valid WhatsApp number.':duplicate?'This WhatsApp number is already used by '+duplicate.name+'.':!password?'Enter a password of at least 12 characters.':password.length<12?'Password must be at least 12 characters.':'';
    ...
    form.onsubmit=async event=>{
      ...
      try {
        ...
        await done();
      } catch(error) {const status=form.querySelector('[role="status"]');busy=false;save.textContent=member?'Save changes':'Create user';update();status.textContent=error.message;status.focus();}
    };
