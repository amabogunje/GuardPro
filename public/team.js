...
export function teamSettings(host,{site,users,api,esc,icon,done}) {
  ...
  function edit(member) {
    ...
    form.onsubmit=async event=>{
      ...
      try {
        ...
        await api('/api/admin',body);await done();
      } catch(error) {form.querySelector('[role="status"]').textContent=error.message;busy=false;save.textContent=member?'Save changes':'Create team member';update();}
    };
