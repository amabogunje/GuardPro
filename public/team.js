...
export function teamSettings(host,{site,users,reusableUsers=[],api,esc,icon,done,roleOnly=null,allowReuse=false,staffMode=false}) {
  ...
  function list(focusId) {
    host.innerHTML=`<section class="card checkpoint-panel team-panel"><div class="checkpoint-heading"><h2>${staffMode?'Staff':'Users'} <span class="checkpoint-count">(${members.length})</span></h2><span class="team-actions"><button type="button" class="checkpoint-text-action" id="add-member">+ Add ${staffMode?'staff':'user'}</button>${allowReuse?`<button type="button" class="checkpoint-text-action" id="reuse-member">+ Reuse${staffMode?' staff':''}</button>`:''}</span></div>${members.length>size?`<label class="checkpoint-search">Find ${staffMode?'staff':'a user'}<input type="search" id="team-search" placeholder="Name, number or email" value="${esc(query)}"></label>`:''}<div id="team-rows"></div></section>`;
  ...
  function reuse() {
    host.innerHTML=`<section class="card checkpoint-panel team-panel"><div class="checkpoint-heading"><h2>Reuse existing ${roleOnly?'supervisor':staffMode?'staff':'user'}</h2><button type="button" class="checkpoint-text-action" id="team-cancel">Cancel</button></div>${reusable.length?`<form id="reuse-editor" class="settings-fields"><label>${roleOnly?'Supervisor':staffMode?'Staff member':'User'}<select name="user_id">${reusable.map(member=>`<option value="${esc(member.id)}">${esc(member.name)} · ${member.role==='guard'?'Guard':'Supervisor'}${member.whatsapp?' · '+esc(member.whatsapp):''}</option>`).join('')}</select></label><p class="muted">This keeps the person’s account and adds access to this property.</p><button class="primary">Add to this property</button><p class="checkpoint-status" role="status"></p></form>`:`<p class="checkpoint-empty">No eligible ${staffMode?'staff':'users'} from your other properties are available.</p>`}</section>`;
