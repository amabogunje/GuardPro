...
The owner home now shows **Guard coverage health**, **Patrol coverage health** and **Report risk signal**. The cards are display-only for MVP. The period is the last seven days, including activity already due today; later shift starts and patrol slots are not counted as missed. Unknown roster days and Any-guard attendance without a required headcount are excluded, not scored as perfect. Guard health is `100 × (on-time + 0.5 × late) / expected`, rounded to a whole percent, with a five-minute late allowance. A missed record means no matching start has been received; it does not prove absence. Patrol health combines completed checkpoint coverage (70%) and on-time patrol starts (30%); unavailable checkpoint evidence leaves the score unknown. Delayed uploads can revise results.
