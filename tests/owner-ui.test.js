...
test('owner KPIs are display-only and supervisor mode persists without changing identity',{concurrency:false},async()=>{
 ...
 try {
  ...
  await p.getByRole('button',{name:'Add User',exact:true}).click();await p.getByRole('button',{name:'I supervise this property',exact:true}).click();await p.locator('.owner-overflow-menu summary').click();await p.getByRole('button',{name:'Act as supervisor',exact:true}).waitFor();assert.equal(await p.getByRole('button',{name:'Open supervisor view',exact:true}).count(),0);await p.getByRole('button',{name:'Act as supervisor',exact:true}).click();await p.getByRole('button',{name:'Settings',exact:true}).waitFor();assert.equal(await p.locator('.topbar > [data-action="logout"]').count(),0);await p.locator('.owner-overflow-menu summary').click();await p.getByRole('button',{name:'Return to owner view',exact:true}).waitFor();const stateResponse=p.waitForResponse(r=>r.url().endsWith('/api/state')&&r.status()===200);await p.reload();await p.locator('.owner-overflow-menu summary').click();await p.getByRole('button',{name:'Return to owner view',exact:true}).waitFor();await p.getByRole('button',{name:'Settings',exact:true}).waitFor();await p.getByRole('button',{name:'Settings',exact:true}).click();await p.getByRole('tab',{name:'Users',exact:true}).click();await p.getByRole('button',{name:'+ Add user',exact:true}).click();const team=p.locator('#team-editor');await team.locator('[name="name"]').fill('WhatsApp-only owner user');await team.locator('[name="whatsapp"]').fill('+1 212 555 0199');await team.locator('[name="password"]').fill('Owner-WhatsApp-2026!');assert.equal(await team.getByRole('button',{name:'Create user',exact:true}).isEnabled(),true);await team.getByRole('button',{name:'Create user',exact:true}).click();await p.getByRole('button',{name:'Edit WhatsApp-only owner user',exact:true}).waitFor();
