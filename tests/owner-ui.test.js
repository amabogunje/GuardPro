...
test('owner mobile setup, supervisor creation and honest subscription screen',{concurrency:false},async()=>{
 ...
 try {
  await geometry(p);
  for(const name of ['Manage Property','Add Staff','View Subscription'])assert.equal(await p.getByRole('button',{name,exact:true}).count(),1);
  await p.getByRole('button',{name:'Manage Property',exact:true}).click();await p.getByRole('button',{name:'Set property location',exact:true}).click();
  ...
  await p.getByRole('button',{name:'Home',exact:true}).click();await p.getByRole('button',{name:'Add Staff',exact:true}).click();await p.getByRole('button',{name:'+ Add staff',exact:true}).click();
  const team=p.locator('#team-editor');assert.equal(await team.locator('[name="role"] option').count(),2);await team.locator('[name="role"]').selectOption('supervisor');await team.locator('[name="name"]').fill('Fictional Test Supervisor');await team.locator('[name="email"]').fill('new-owner-ui@example.test');await team.locator('[name="password"]').fill('Owner-UI-Test-2026!');await team.getByRole('button',{name:'Create staff member',exact:true}).click();await p.getByRole('button',{name:'Edit Fictional Test Supervisor',exact:true}).waitFor();
  await p.getByRole('button',{name:'Home',exact:true}).click();await p.getByRole('button',{name:'View Subscription',exact:true}).click();await p.getByRole('heading',{name:'Free pilot',exact:true}).waitFor();await p.getByText('Up to 5',{exact:true}).waitFor();await geometry(p);
