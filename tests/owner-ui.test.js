...
test('owner KPIs are display-only and supervisor mode persists without changing identity',{concurrency:false},async()=>{
 ...
 try {
  await p.locator('.owner-kpi').first().waitFor();assert.equal(await p.locator('.owner-kpi').count(),3);assert.equal(await p.locator('.owner-kpi details, .owner-kpi summary, .owner-kpi button, .owner-kpi a').count(),0);await p.getByText('Last seven days',{exact:true}).waitFor();
