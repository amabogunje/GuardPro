...
test('owner mobile setup, supervisor creation and honest subscription screen',{concurrency:false},async()=>{
 ...
 try {
  await geometry(p);
  for(const name of ['Manage Property','Add Supervisors','View Subscription'])assert.equal(await p.getByRole('button',{name,exact:true}).count(),1);
  ...
  await p.getByRole('button',{name:'Home',exact:true}).click();await p.getByRole('button',{name:'Add Supervisors',exact:true}).click();await p.getByRole('button',{name:'+ Add supervisor',exact:true}).click();
