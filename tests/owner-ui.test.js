...
test('owner mobile setup, supervisor creation and honest subscription screen',{concurrency:false},async()=>{
 ...
 try {
  ...
  await p.getByRole('button',{name:'Home',exact:true}).click();await p.getByRole('button',{name:'Add User',exact:true}).click();await p.getByRole('button',{name:'+ Add user',exact:true}).click();
