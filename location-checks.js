...
export function propertyInput(body) {
  const address=String(body.address||'').trim(),latitude=Number(body.latitude),longitude=Number(body.longitude),radius_m=Number(body.radius_m);
  ...
  return {address,latitude,longitude,radius_m};
}
