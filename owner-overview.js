...
export function ownerOverview({site,users,supervisors,plans,shifts,events,incidents,checkpoints,locations,reviews,resolutions,now=Date.now()}) {
  ...
  return {
    ...
    setup:{propertyConfigured:!!property,address:property?.address||'',supervisorConfigured:!!supervisors.length},
    supervisors,
    ...
    attentionRequired:!!(missing||open||groups.length||patrols.needsReview||!property||!supervisors.length)
  };
}
