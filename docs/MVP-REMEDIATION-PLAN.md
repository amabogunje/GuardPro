...
**Current work:** Repair the reopened X7 workflow-test isolation regression, then obtain independent review for completed supervisor, guard and O1 self-service remediations and prepare signed cross-role Android validation.
**Next issue:** R-001 / X7: isolate the workflow fixture/server so the complete release suite remains deterministic.
**Progress:** S0 is verified locally and remains Not released. S1–S7, G1–G5 and O1 remain ready for independent verification. G6 is blocked on real-device evidence across all roles; G7 is ready for independent verification. X7 is reopened after a clean-port release run failed within the workflow fixture.
...
| X7 | P1 | Reopened | Coherent enabled-feature release suite, independent fixtures, baseline failure ledger and current docs/runbooks. CI/repeatable command preserves regression tests. | The prior 58/58 result is superseded: a clean-port run reached 45 passes then the workflow fixture observed unexpected active-shift state and lost its local server, yielding 17 failures. R-001 tracks deterministic test isolation before release evidence can be accepted. |
...
|---|---|---|---|---|---|
| R-001 | P1 | X7 release verification | Open | From a confirmed clear fixed-port state, run `npm run test:release`. `tests/workflows.test.js` can receive pre-existing active-shift state at its first start and later loses `127.0.0.1:3101`, making subsequent cases fail with connection refused. The same workflow suite passed when run independently after its port was clear. | Make the complete suite deterministic with no shared/orphaned workflow server or fixture state; retain all workflow assertions and demonstrate a clean full-suite result. Engineering owns the fix; QA independently repeats it. |
