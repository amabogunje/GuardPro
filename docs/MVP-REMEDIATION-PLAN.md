...
**Current work:** Remaining shared release gates: guard data minimization, recovery/offboarding, support and pilot-capacity boundaries.
**Next issue:** X3: agree and rehearse database-plus-media recovery with the named recovery owner.
...
| D4 Urgent help | Remove dummy alarm; use configured normal telephone help, no guaranteed monitoring | Accepted 2026-09-17; product owner | G3, S4, X8 |
...
Next landing action: commit, deploy and record the actual deployment smoke result requested by the product owner; then obtain user feedback on the public draft. Default-root integration remains a separate delivery choice. Existing X3 recovery next action and all release gates remain unchanged. X8 remains In progress; O1/O8 are not reclassified by this work. Release state: **Not released**.
