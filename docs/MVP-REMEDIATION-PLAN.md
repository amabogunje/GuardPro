...
| 2026-09-18 | R-014, R-013 | A password-reset owner could be blocked by an old locally encrypted vault even after server authentication succeeded. Added reset-time vault-key rotation and a narrowly owner-only recovery path that creates a new encrypted vault while retaining the old encrypted record. | Focused owner reset/vault tests passed 3/3, including a mobile-browser recreation of the legacy encrypted-vault failure. Guard and supervisor vault behavior remains unchanged so pending evidence is never silently replaced. Ready for independent verification and deployment. |
| 2026-09-18 | R-014, R-013 | Pushed `dde28e5` and deployed the owner vault-recovery correction. | Production: `dpl_3Wihbr3aQx2DSSVqZ5WZUuhLnVKn` at `https://guardpro-qpc44cykb-mabogunje-7146s-projects.vercel.app`, aliased to `https://getguardpatrol.com`. Production health passed and reports `ownerPasswordResetAvailable:true`. Independent real-owner reset/sign-in remains required before R-014 is Verified. |
...
| R-014 | P1 | R-013 owner reset vault rotation | Ready for independent verification | An owner who reset their server password could not sign in on a browser holding a vault encrypted with the old password. | A reset rotates the owner vault account key, retaining the old encrypted record. A legacy owner vault mismatch can rotate to a fresh owner vault only after a successful online owner login; guards and supervisors still require the old password to protect pending evidence. Focused regression passed 3/3, including a mobile-browser legacy-vault reproduction. Repeat an actual Production owner reset/sign-in before marking Verified. |
