...
| R-013 | P1 | G5 / O6 account recovery | Ready for verification | A person who forgot their password or sign-in identifier had no in-app guidance, and an authorized manager had no clearly labelled password-reset action. | The sign-in page provides a clear recovery route to ISDL support or the assigned manager. An authorized manager can set a new temporary password in Users; all sessions are revoked and the reset is audited. Independent production-browser and recovery-process verification is still required. |
