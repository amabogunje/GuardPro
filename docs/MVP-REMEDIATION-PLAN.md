...
| R-012 | P1 | O6 user provisioning | Ready for verification | The Add user form visually presented name, role and WhatsApp number as sufficient, but a hidden required password kept Create team member disabled. | A valid name and WhatsApp number now enable Create user. If no password is entered, the server creates a unique temporary password and returns it once to the authorized creator; the user can then sign in with their WhatsApp number. Independent production-browser verification is still required. |
