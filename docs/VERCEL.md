...
Owner self-service password recovery additionally requires `RESEND_API_KEY` and a verified sender domain. The Vercel Resend integration provides `RESEND_API_KEY` and `RESEND_EMAIL_DOMAIN`; Guard Patrol derives `Guard Patrol <no-reply@your-domain>` from that domain. `RESEND_FROM` can override that address. Set `PASSWORD_RESET_BASE_URL=https://getguardpatrol.com` for Production only if a custom route is needed; Preview otherwise uses its deployment URL. Until the email variables and reset-token migration are available, the sign-in help screen honestly directs owners to ISDL support and does not say an email was sent.
